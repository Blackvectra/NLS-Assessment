...
The NLS Assessment Tool requires read-only permissions across Microsoft Graph, Exchange Online, SharePoint, and Teams. **No write permissions are required or requested.**
...
```
display name:  NLS Assessment (Read-Only)
UPN:           nls-assessment@<tenant>.onmicrosoft.com
