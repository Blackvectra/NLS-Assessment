...
| 21 | Branding leak: 38 client assessment HTML/JSON files tracked in NRG `output/` | Privacy | `ndaco.org`, `nextlayersec.io` real tenant configs in main | `.gitignore` only excluded `Reports/`, not `output/` | Added `output/` to `.gitignore` + `git rm -r --cached output/` | PR #29 |
