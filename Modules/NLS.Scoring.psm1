...
function Invoke-NLSScoringModel {
    ...
    if ($dmarcData -and $dmarcData['DMARC']) {
        ...
        $allEnforced  = ($dmarc.Domains | Where-Object { -not $_.Enforced }).Count -eq 0
        $noneMissing  = $dmarc.MissingCount -eq 0

        if ($allEnforced -and $noneMissing) {
            Add-NLSFinding -ControlId 'DMARC' -State 'Satisfied' `
                -Detail "DMARC at p=reject on all $($dmarc.Domains.Count) domain(s)." `
                -CurrentState 'p=reject on all domains' -Recommended 'p=reject on all domains'
        } elseif (-not $noneMissing) {
            Add-NLSFinding -ControlId 'DMARC' -State 'Gap' `
                -Detail "$($dmarc.MissingCount) domain(s) have no DMARC record." `
                -CurrentState "Missing on $($dmarc.MissingCount) domain(s)" `
                -Recommended 'p=reject on all domains' `
            ...
            Add-NLSFinding -ControlId 'DMARC' -State 'Partial' `
                -Detail "$($dmarc.NoneCount) domain(s) at p=none (monitoring only). $($dmarc.QuarantineCount) at p=quarantine." `
                -CurrentState "p=none on $($dmarc.NoneCount) domain(s)" `
                -Recommended 'p=reject on all domains' `
                -Remediation 'Advance DMARC policy from p=none to p=quarantine then p=reject after validating SPF and DKIM alignment'
        }
    }
    ...
    if ($mfData) {
        ...
        if ($mfData['MTASTS']) {
            $mtas = $mfData['MTASTS']
            if ($mtas.EnabledCount -eq $mtas.TotalDomains -and $mtas.TotalDomains -gt 0) {
                Add-NLSFinding -ControlId 'MTASTS' -State 'Satisfied' `
                    -Detail "MTA-STS published on all $($mtas.TotalDomains) domain(s)." `
                    -CurrentState 'Enabled on all domains' -Recommended 'Enabled on all domains'
            } else {
                Add-NLSFinding -ControlId 'MTASTS' -State 'Gap' `
                    -Detail "MTA-STS not published. SMTP downgrade attacks possible." `
        ...
        if ($mfData['MalwareFilter']) {
            ...
            if ($malware.Hardened) {
                Add-NLSFinding -ControlId 'MalwareFilterPolicy' -State 'Satisfied' `
                    -Detail "Malware filter set to delete infected messages. ZAP enabled." `
                ...
                Add-NLSFinding -ControlId 'MalwareFilterPolicy' -State 'Gap' `
                    -Detail "Malware filter action: $($malware.Action). ZAP enabled: $($malware.ZapEnabled)." `
                    -CurrentState "Action: $($malware.Action), ZAP: $($malware.ZapEnabled)" `
